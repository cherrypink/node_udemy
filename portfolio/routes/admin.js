...
router.post('/add', upload.single('projectimage'), function(req,res,next){
	...
	var query = connection.query('INSERT INTO projects SET ?', 
									project, 
									function(err,result){
...
router.get('/edit/:id', function(req,res,next){
	connection.query("SELECT * FROM projects WHERE id = ?", req.params.id, function(err, rows, field){
		if(err) throw err;
...
router.post('/edit/:id', upload.single('projectimage'), function(req,res,next){
	...
	if(req.file){
		...
		var projectImageName = req.body.filepath;
	}
	...
	var query = connection.query('UPDATE projects SET ? WHERE id = '+req.params.id, 
									project, 
									function(err,result){
										console.log("Error" + err);
